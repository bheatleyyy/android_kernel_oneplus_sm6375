	...
	struct regmap *regmap)
{
	...
	for (i = 0; i < ARRAY_SIZE(trimming_regs); i++) {
		if (0 != sia81xx_regmap_read(regmap, 
			trimming_regs[i].addr, 1, (char *)&vals[i]))
			return ;
	}

	crc = vals[ARRAY_SIZE(trimming_regs) - 1];
	if (crc != crc8_maxim(vals, ARRAY_SIZE(trimming_regs) - 1)) {
		...
		*vals |= 0x02;
		...
		for (i = 0; i < ARRAY_SIZE(trimming_regs) - 1; i++)
			sia81xx_regmap_write(regmap, 
